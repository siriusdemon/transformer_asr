...
from utils import get_seq_mask, get_dec_seq_mask
from module import LayerNorm, PositionalEncoding

from attention import MultiHeadedAttention
from module import LayerNorm, PositionwiseFeedForward
...
#        print(target_length.shape,memory.shape,memory_mask.shape)
#        print(targets.shape)
        dec_output = self.embedding(targets)
#        print(91,dec_output.shape)
        dec_output = self.pos_encoding(dec_output)
#        print(92,dec_output.shape)
        dec_mask = get_dec_seq_mask(targets, target_length)
#        print(51,dec_output.shape,dec_mask.shape,memory.shape,memory_mask.shape)
        for _, block in enumerate(self.blocks):
            dec_output, dec_mask = block(dec_output, dec_mask, memory, memory_mask)
#        print(94,dec_output.shape,dec_mask.shape)
        if self.normalize_before:

...
from module import *
from utils import get_enc_padding_mask#, get_streaming_mask
...
#        print(inputs.size(),input_length.size(),enc_mask.size(),90)
        enc_output, enc_mask = self.embed(inputs, enc_mask)
#        print(enc_output.size(),enc_mask.size(),91)
        enc_output.masked_fill_(~enc_mask.transpose(1, 2), 0.0)
#        print(enc_output.size(),92)
        # if streaming:
        ...
        for _, block in enumerate(self.blocks):
            ...
            enc_output.masked_fill_(~enc_mask.transpose(1, 2), 0.0)
#        print(enc_output.size(),enc_mask.size(),93)
        if self.normalize_before:
            enc_output = self.after_norm(enc_output)
#        print(enc_output.size(),94)
#        sys.exit()

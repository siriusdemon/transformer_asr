...
def mask_finished_scores(score, flag):
    ...
    if beam_width > 1:
        unfinished = torch.cat((zero_mask, flag.repeat([1, beam_width - 1]).bool()), dim=1)
...
class TransformerRecognizer(object):
    def __init__(self, model, lm=None, unit2char=None, beam_width=5, max_len=100, 
                 penalty=0.6, lamda=5, use_cuda=False):
    ...
    def recognize(self, inputs, inputs_length):
        ...
        with torch.no_grad():
            ...
            max_indices += torch.arange(b, dtype=torch.long, device=self.device) * self.beam_width
    ...
    def predict(self, wav, num_mel_bins=40, apply_normalize=True):
    ...
    def decode_step(self, preds, enc_state, enc_mask, scores, flag):
        ...
        base_k_indices = torch.arange(batch_size, device=self.device).view(-1, 1).repeat([1, self.beam_width])
